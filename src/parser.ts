...
export class Parser {
	...
	public FindSingleLineComments(activeEditor: vscode.TextEditor): void {
		...
		while ((match = regEx.exec(text))) {
			...
			let matchText = match[3] as string;
			let matchTag = this.tags.find((item) => {
				let tagReg = item.tag;
				if (item.isRegex) {
					return new RegExp(tagReg, 'i').test(matchText);
				} else {
					return tagReg.toLowerCase() === matchText.toLowerCase();
				}
			});
		...
		while ((match = regEx.exec(text))) {
			...
			while ((line = commentRegEx.exec(commentBlock))) {
				...
				let matchTag = this.tags.find((item) => {
					let tagReg = item.tag;
					if (item.isRegex) {
						return new RegExp(tagReg, 'i').test(matchString);
					} else {
						return tagReg.toLowerCase() === matchString.toLowerCase();
					}
				});
